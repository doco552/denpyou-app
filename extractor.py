...
【表記ゆれ統一ルール】
- 伝票に「〇〇クーラー」「クーラー」など「クーラー」という表記がある場合、item_name 内の該当箇所は必ず「エアコン」に置き換えて出力すること
  （例: 「ダイキンクーラー」→「ダイキンエアコン」、「クーラー」→「エアコン」）
...
- 「エアコン」「クーラー」の品名の後に続く能力値（例: 2.8、px など）や状態メモ（例: 配管折れ、動作確認 など）は抽出対象外とすること
  → item_name には純粋な品名のみを入れ、これらの付加情報は item_name にも model_number にも含めないこと
  → features にも入れず、完全に無視すること
