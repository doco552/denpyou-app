...
━━━━━━━━━━━━━━━━━━━━━━━━━━
【最優先・絶対禁止事項】これらは例外なく厳守すること
━━━━━━━━━━━━━━━━━━━━━━━━━━
■ インボイス登録番号の完全破棄
  「T」から始まる13桁の数字（例: T1234567890123）、および
  「インボイス」「登録番号」「適格請求書」などの印字・手書き文字は
  税務上の番号であり商品データではない。
  → item_name / features / model_number を含む「いかなる項目にも絶対に出力しないこと」
  → 伝票上で見つけても完全に無視（破棄）すること

■ ノイズの引き算（迷ったら捨てる）
  エアコン行の features に入れてよいのは
  「メーカー名」「能力値（2.2や2.8などの小数）」「製造年（〇〇年製）」の3要素のみ。
  これら以外の文字・記号（「配管折れ」「キズ」「px」「動作確認」など）は
  → features にも model_number にも含めず、迷わず捨てること（空白 / null）
━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- 年式（製造年）の表記統一：伝票上の記載が「2022年製」「22製」「'22」「22'」など、いかなる形式であっても、出力は必ず「西暦の下2ケタ＋年」の形式（例: 「22年」）に統一すること
  （変換例: 「2021年製」→「21年」、「22製」→「22年」、「'21」→「21年」）
...
【品目名の統一ルール】（表記ゆれを防ぐため、以下のマッピングを必ず適用すること）
- 「2ドア冷蔵庫」「3ドア冷蔵庫」「ドア冷蔵庫」など「〇ドア冷蔵庫」系 → item_name = 「冷蔵庫」
- 「ドラム式洗濯機」「縦型洗濯機」など「〇〇洗濯機」系 → item_name = 「洗濯機」
- 「電子レンジ」「オーブンレンジ」など「〇〇レンジ」系 → item_name = 「レンジ」
- 「ガスコンロ」「IHコンロ」「LPコンロ」など「〇〇コンロ」系 → item_name = 「コンロ」
- 「シーリング」「照明」「シーリングライト」など照明器具 → item_name = 「照明」
- 「ジャー」「炊飯ジャー」「電気炊飯器」など炊飯系 → item_name = 「炊飯器」
- 「調理家電」「小物家電山」など小物家電まとめ書き → item_name = 「小物家電」
...
- features にはスペース区切りで次の3要素のみをまとめて入れること：
  ...
  3. 製造年（例: 21製 / 2021年製 / '21 → いずれも「21年」に変換して出力）
  → 出力例: features = 「パナソニック 2.8 21年」
...
- 型番（model_number）は商品固有の型式番号のみ。「T」から始まる13桁のインボイス番号は絶対に含めないこと。型番が存在しなければnull
