...
【表記ゆれ統一ルール】
- 伝票に「〇〇クーラー」「クーラー」など「クーラー」という表記がある場合、item_name は必ず「エアコン」のみにすること
  （例: 「ダイキンクーラー」→ item_name=「エアコン」、「クーラー」→ item_name=「エアコン」）
  ※メーカー名はitem_nameではなくfeaturesに入れること（下記「エアコン専用ルール」参照）
...
【エアコン専用ルール】（「エアコン」「クーラー」と読み取れる商品に適用）
- item_name は必ず「エアコン」のみとする
- features にはスペース区切りで以下をまとめて入れること：
  1. メーカー名（例: パナソニック、ダイキン、日立 など）
  2. 能力値：「2.2」「2.8」など小数点を含む数字。手書きのかすれで後ろに「k」「kw」「px」などが付いていても、数字部分（例: 2.8）のみを採用すること
  3. 製造年（例: 21製、2021年製 など）
  → 出力例: features = 「パナソニック 2.8 2021年製」
- 上記以外の付加情報（「配管折れ」「動作確認済」などの状態メモ、「px」などの判読不明文字）は一切含めないこと
- model_number は null にすること
